...
export function Sidebar({ projectId }: { projectId: string }) {
    ...
    const handleAddFile = async (type: "file" | "folder") => {
        ...
        const result = await createFile(projectId, parentId, name, type, id);
    ...
    const handleRenameSubmit = async () => {
        ...
        const originalName = files.find(f => f.id === renamingId)?.name || "";
        ...
        const result = await renameFileAction(renamingId, renameValue);

        if (result?.error) {
            // Revert
            renameFile(renamingId, originalName);
            console.error(result.error);
        }
    };
